...
First, clone the MaxText repository by following the [MaxText Getting Started Guide](https://github.com/AI-Hypercomputer/maxtext#getting-started):

Navigate to the root of your local MaxText repository and submit the job using `gcluster job submit`. You can pass your local directory (`--build-context .`) along with a prebuilt base image so `gcluster` compiles and pushes your latest code changes without requiring a local Docker daemon:
