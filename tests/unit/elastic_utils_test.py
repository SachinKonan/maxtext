...
class ElasticUtilsTest(parameterized.TestCase):
  ...
  def test_elastic_checkpoint_guard_checks_scale_up_on_success(self):
    ...
    with self.assertRaises(ScaleUpSignalError):
      with elastic_utils.elastic_checkpoint_guard(config, mock_checkpoint_manager):
        pass

    mock_checkpoint_manager.wait_until_finished.assert_called_once()

  def test_elastic_checkpoint_guard_none_manager(self):
    ...
    with self.assertRaises(ScaleUpSignalError):
      with elastic_utils.elastic_checkpoint_guard(config, checkpoint_manager=None):
        pass

  def test_elastic_checkpoint_guard_skips_scale_up_on_failure(self):
    ...
    with elastic_utils.elastic_checkpoint_guard(config, self.fake_manager, handler):
