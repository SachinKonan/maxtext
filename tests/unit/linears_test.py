...
class MlpBlockTest(unittest.TestCase):
  ...
  def test_dense_fsdp_two_stage_all_gather_tpu_only(self):
    ...
    def build_and_run(use_two_stage):
      config_arguments = {
          "per_device_batch_size": 1.0,
          "run_name": "test",
          "enable_checkpointing": False,
    ...
    reference = build_and_run(use_two_stage=False)
    two_stage = build_and_run(use_two_stage=True)
    ...
    self.assertTrue(
        ...
        msg=(
            "two-stage all-gather changed MLP output: max abs diff="
