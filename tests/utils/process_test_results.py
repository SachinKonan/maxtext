...
REL_REGRESSION_TRAIN_COMPILE_PCT = 1.50  # 50% slower
REL_REGRESSION_OTHER_PCT = 1.20  # 20% slower

# Absolute increase thresholds to filter out noise
ABS_INCREASE_UNIT_SEC = 15.0
ABS_INCREASE_INTEGRATION_SEC = 30.0
ABS_INCREASE_TRAIN_COMPILE_SEC = 120.0
...
def process_testcase(testcase, xml_file, baseline_data, new_baseline_data):
  ...
  if is_integration:
    ...
    rel_regression_pct = REL_REGRESSION_OTHER_PCT
    test_type = "Integration Test"
  elif is_train_compile:
    abs_noise_threshold = ABS_INCREASE_TRAIN_COMPILE_SEC
    rel_regression_pct = REL_REGRESSION_TRAIN_COMPILE_PCT
  ...
  # Skip regression checking for CPU and train-compile tests due to shared CPU multi-tenancy noise
  skip_regression = is_cpu or is_train_compile
  ...
  if not skip_regression and full_name in baseline_data:
    ...
    if base_time > 0:
      ...
      if ratio >= rel_regression_pct and increase > abs_noise_threshold:
        ...
        print(f"  Thresholds: >{(rel_regression_pct - 1) * 100:.0f}% AND >{abs_noise_threshold}s")
        print("-" * 50)
        failed = True
