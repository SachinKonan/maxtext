...
def dequantize_fp8(x: torch.Tensor, s: torch.Tensor, block_size: int = 128) -> torch.Tensor:
  ...
  if M % block_size != 0 or N % block_size != 0:
    s_expanded = torch.repeat_interleave(s, block_size, dim=0)
    s_expanded = torch.repeat_interleave(s_expanded, block_size, dim=1)
    s_expanded = s_expanded[:M, :N]
    return (x * s_expanded.to(torch.float32)).to(torch.bfloat16)

  # Reshape and broadcast
  q = x.reshape(M // block_size, block_size, N // block_size, block_size)
  scale = s.to(torch.float32).reshape(M // block_size, 1, N // block_size, 1)

  return (q * scale).to(torch.bfloat16).reshape(M, N)


# reference: huggingface/transformers/blob/main/src/transformers/integrations/finegrained_fp8.py
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--input-path", "--input-fp8-hf-path", type=str, required=True, help="Path to DeepSeek FP8/FP4 Hugging Face folder"
  )
  parser.add_argument(
      "--output-path", "--output-bf16-hf-path", type=str, required=True, help="Directory to save output BF16 weights"
  )
  parser.add_argument(
      "--cache-size", "--cache-file-num", type=int, default=2, help="Max cached files in RAM during indexing lookup"
  )
