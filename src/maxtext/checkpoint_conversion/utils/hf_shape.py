...
def QWEN3_5_HF_WEIGHTS_TO_SHAPE(config):
  ...
  num_attention_heads = config["num_attention_heads"]
  num_key_value_heads = config["num_key_value_heads"]
  # Dense Qwen3.5 variants (e.g. Qwen3.5-27B) have no expert fields; their MLP
  # is a standard SwiGLU block sized by `intermediate_size`.
  num_experts = int(config.get("num_experts") or 0)
