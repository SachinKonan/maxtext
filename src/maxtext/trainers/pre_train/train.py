...
def loss_fn(
  ...
  if isinstance(model, nn.Module):
    ...
    if config.mtp_num_layers > 0:
      intermediate_outputs["mtp_losses"] = nnx.pop(
          model, mtp_losses
      ).to_pure_dict()
      intermediate_outputs["mtp_acceptance"] = nnx.pop(
          model, mtp_acceptance
      ).to_pure_dict()
...
def recover(
  ...
  if metric_logger_instance is not None:
    metric_logger_instance.clear_metrics()
  ...
  while True:
    try:
      ...
      elastic_manager.available_inactive_slices = frozenset()
      ...
      with jax.default_device(elastic_manager.default_device):
        ...
        if existing_checkpoint_manager is not None:
          elastic_utils.cancel_unfinalized_checkpoint(existing_checkpoint_manager)
...
def train_loop(config, recorder, state=None):
  ...
  if config.elastic_enabled:
    ...
    elastic_manager.start_monitoring(config.elastic_new_slice_check_period)
    elastic_utils.elastic_manager = elastic_manager
    devices = elastic_utils.live_devices(config)
  else:
