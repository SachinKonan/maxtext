...
class DenseGeneral(nnx.Module):
  ...
  def _maybe_two_stage_all_gather(self, kernel):
    ...
    """
    # Stage axes per supported MLP kernel layout; fused_mlp
    # ("embed", "num_activations", "mlp") is absent and thus skipped below.
    stage_axes = {
        ("embed", "mlp"): (("embed_fsdp", "mlp_no_fsdp"), (None, "mlp_no_fsdp")),
        ("mlp", "embed"): (("mlp", None), ("mlp_no_fsdp", None)),
    }.get(self.kernel_axes)
    if (
        stage_axes is None
        or not self.use_two_stage_all_gather
