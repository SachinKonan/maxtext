...
def _verify_lora_parameters(lora_model: nnx.Module, mt_config: pyconfig.HyperParameters) -> None:
  ...
  print(f"DEBUG_JETS: _verify_lora_parameters: lora_model id={id(lora_model)}, is_lora_enabled={enabled}")
  for path, value in nnx.iter_graph(lora_model):
    print(f"DEBUG_JETS: _verify_lora_parameters: graph node path={path}, type={type(value)}, id={id(value)}")
    if isinstance(value, nnx.LoRAParam):
      print(f"DEBUG_JETS: _verify_lora_parameters: FOUND LoRAParam at {path}")
  if enabled:
    wrapped_modules = set()
    for path, value in nnx.iter_graph(lora_model):
      if isinstance(value, nnx.LoRAParam):
